...
echo ""
echo "[INFO] Step 1: Checking for required tools..."

for tool in gcloud gemini; do
...
echo ""
echo "[INFO] Step 2: Setting up Google Cloud project..."
read -p "Please enter your Google Cloud Project ID: " PROJECT_ID

# Log in to gcloud if not already
echo ""
echo "[INFO] Logging into gcloud..."
gcloud auth list | grep ACTIVE &> /dev/null || gcloud auth login
...
# Enable Gemini API
echo ""
echo "[INFO] Enabling Gemini API..."
gcloud services enable cloudaicompanion.googleapis.com

# Log in to Gemini CLI
echo ""
echo "[INFO] Logging into Gemini CLI..."
gemini login
gemini whoami

echo ""
echo "[INFO] Step 3: Creating role-based directory structure..."
mkdir -p tutorschool foodtrade dev
echo "[INFO] Directories created: tutorschool/, foodtrade/, dev/"

# Create GEMINI.md
echo ""
echo "[INFO] Step 4: Creating GEMINI.md configuration..."
cat <<'EOF' > GEMINI.md
...
```
EOF
echo "[INFO] GEMINI.md created successfully."

echo ""
echo "[INFO] Step 5: Creating role-based .todo.md files..."
for role in tutorschool foodtrade dev; do
  cat <<EOF > ./$role/.todo.md
# To-Do List for $role
...
echo "[INFO] .todo.md files created in each role directory."

echo ""
echo "[INFO] Step 6: Creating .env file for API keys..."
cat <<EOF > .env
...
EOF
echo "[INFO] .env file created. Please fill in your API keys."

echo ""
echo "[SUCCESS] AI Agent setup is complete!"
echo "Next steps:"
echo "1. Review the generated GEMINI.md file."
echo "2. Add your API keys to the .env file."
echo "3. Run 'source .env' to load the environment variables."
echo ""